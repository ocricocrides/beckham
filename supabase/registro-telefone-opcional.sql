-- Telefone opcional e livre no registro pelo site.
-- Vazio passa e qualquer texto passa também: sem regra de tamanho ou de "número real". O valor é
-- guardado como foi digitado (só sem espaços nas pontas, cortado em 20 caracteres). A coluna
-- continua NOT NULL e o vazio é gravado como ''.
--
-- A assinatura não muda, então o create or replace mantém as permissões (só authenticated executa).
-- Já aplicado no banco de produção (migration "registro_telefone_livre").
...
declare
  ...
  v_telefone text := left(trim(coalesce(p_telefone, '')), 20);
